...
def resolver_labirinto(labirinto, tela, tamanho_janela=910, taxa_atualizacao=1):
    ...
    for x in range(labirinto.labirinto.shape[0]):
        for y in range(labirinto.labirinto.shape[1]):
            cor = (255, 255, 255) if labirinto.labirinto[x, y] == 0 else (0, 0, 0)
            pygame.draw.rect(tela, cor, (deslocamento_x + y * tamanho_celula, deslocamento_y + x * tamanho_celula, tamanho_celula, tamanho_celula))
    pygame.display.flip()
